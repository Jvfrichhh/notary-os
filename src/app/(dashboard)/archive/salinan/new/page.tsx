...
import { createSalinan } from "@/app/(dashboard)/archive/actions";
import { getDocumentOptionsForDeed } from "@/lib/archiveDocuments";

export default async function NewSalinanPage({ searchParams }: { searchParams: { deedId?: string } }) {
  ...
  return (
    <div className="max-w-2xl space-y-6">
      <div>
        <h1 className="text-xl font-semibold text-foreground">Salinan Baru</h1>
        <p className="text-sm text-muted-foreground">Akta {deed.deedNumber}</p>
      </div>
      <Card>
        <DerivativeForm
          action={createSalinan}
          label="Salinan"
          deeds={[]}
          documents={documents}
