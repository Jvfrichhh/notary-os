...
import { archiveLegalisasi, restoreLegalisasi } from "@/app/(dashboard)/legalisasi/actions";

export const dynamic = "force-dynamic";

export default async function LegalisasiDetailPage({ params }: { params: { id: string } }) {
  ...
  const record = await prisma.legalisasi.findUnique({
    where: { id: params.id },
  ...
  return (
    <div className="max-w-2xl space-y-6">
      <div className="flex items-start justify-between">
        <div>
          <p className="text-xs font-medium text-muted-foreground">Legalisasi</p>
      ...
      {canEdit && (
        <div className="flex items-center gap-2">
          <Link href={`/legalisasi/${record.id}/edit`} className="rounded-md border border-border px-3 py-2 text-sm font-medium text-foreground hover:bg-muted">
            Edit
          </Link>
          {record.recordStatus === "ACTIVE" ? (
            <form action={archiveLegalisasi.bind(null, record.id)}>
            ...
            <form action={restoreLegalisasi.bind(null, record.id)}>
